...
SoftwareSerial mySerialLink(Rx, Tx);  //16 MOSI(RX) 14 MISO(TX)   
...
void OnDataRecv(const uint8_t* mac_addr, const uint8_t* incomingData, int len) {
  ...
  String clientId =  PrintMac(mac_addr);
  ...
  //Write the incoming message to the serial connection
  //===================================================
  //char* Mymessage= (char*) incomingData; 
  //mySerialLink.write(Mymessage);
  ...
  {
    debug("I know this client already: ");
    debugln(clientId);
  }
}
...
void CheckLightsBtn()
{
  if(digitalRead(switchLights)==HIGH)
  {
    SwitchLights=true;
    SendMessage(clientHandler.clientList[clientHandler.CurrentTrainId].Idx);
  }
...
void SendMessage(int index)
{
  ...
  statusDoc["ChangeLights"]= SwitchLights;
  serializeJson(statusDoc, Msg);

  //reset the lights again:
  //======================
  SwitchLights = false;
